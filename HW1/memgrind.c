...
//At the end, the total time is divided by however many iterations were ran, which gives average time of the test
...
//This function is used to get the timings of however many iterations of a single test
//It also checks if the memory has been cleared
double performanceTest(int testNum, int iterations) {
...
//The int parameter 'iterations' is however many iterations a single test will be run
//Each iteration will have its time checked, and then the average time it took for the test to run will be printed
