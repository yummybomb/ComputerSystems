...
void myfree(void *_Memory, char *file, int line) {
   ...
   while (memStart < memEnd) {
      if (memStart == headerStart) {
         if (IsFree(headerStart)) {
            //This indicates that the user is trying to free an already free chunk
            fprintf(stderr,
   ...
   //If any a chunk and the next chunk is free, then the two chunks will be coalesced
   //CoalesceNextChunk() is called to coalesce two adjacent chunks
   memStart = heap;
   while (memStart < memEnd) {
